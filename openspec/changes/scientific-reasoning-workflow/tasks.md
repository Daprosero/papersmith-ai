...
## 11. Final conformance corrective scope

The following pending tasks are an approved corrective scope discovered by the final conformance audit. They supersede the false prior `26/26` completion claim. All T11 tasks remain intentionally unchecked until their implementation and evidence are complete.

### T11.1 — Public scientific workflow wiring
- [x] T11.1 — Public scientific workflow wiring
- **Files/discovery:** `.pi/extensions/proposal-workspace.ts:paper_proposal_v2_execute` composition root and registered-tool coordinator, `.pi/extensions/paper-proposal-v2/exports.ts`, public scientific request/result projection, feature-admission seam, and read-only adapter interfaces.
- **Depends on:** T1.1–T1.3, T4.4, T10.1.
- **Implement:** wire the public registered tool through the composition root/coordinator to the scientific workflow without bypassing the explicit route. Keep `PAPER_PROPOSAL_V2_SCIENTIFIC_WORKFLOW_ENABLED` default-off and read only at outer admission; return the exact disabled scientific result without falling back to a document operation. Preserve lifecycle, direct-document, and `DELIBERATE` route isolation. Expose only read-only adapters before materialization, fail closed on invalid or missing required metadata, and project only minimal allowlisted public identities/statuses/summaries.
- **Tests:** add registered-tool routing fixtures at `.pi/extensions/proposal-workspace.ts:paper_proposal_v2_execute` and `tests/paper-proposal-v2-scientific-routing.test.mjs` covering default-off admission, the exact disabled result, no scientific construction/store access for non-scientific routes, read-only adapter capability negatives, metadata rejection, and bounded privacy-safe public projection. Record the repository-native manual verification because no test runner is configured.
- **Acceptance:** (1) the registered public tool reaches the coordinator only for an explicitly admitted scientific request; (2) absent/false `PAPER_PROPOSAL_V2_SCIENTIFIC_WORKFLOW_ENABLED` returns the exact `unavailable` scientific result and performs no document or scientific mutation; (3) lifecycle, direct-document, and `DELIBERATE` routes retain their existing result contracts and never access scientific components; (4) pre-materialization adapters cannot write documents, manifests, receipts, or revisions; (5) invalid or incomplete metadata fails closed; (6) the public projection contains only minimal bounded allowlisted identities/statuses/summaries and no prompts, private reasoning, raw traces, or unrestricted transcripts; and (7) the user constraint is preserved verbatim: “Amend only `openspec/changes/scientific-reasoning-workflow/tasks.md`; do not modify code, tests, or apply-progress.”

### T11.2 — Public scientific workflow E2E
- [x] T11.2 — Public scientific workflow E2E
- **Files/discovery:** registered-tool entry point `.pi/extensions/proposal-workspace.ts:paper_proposal_v2_execute`, coordinator/service wiring, `.pi/extensions/paper-proposal-v2/exports.ts`, and `tests/paper-proposal-v2-scientific-*.test.mjs` public-tool fixtures.
- **Depends on:** T11.1, T6.3, T7.1–T7.2, T8.1–T9.3.
- **Implement:** exercise the public registered tool end to end with scientific mode enabled: prove deliberation has no document write; prove exact synthesis modification/reopen and user acceptance semantics; prove explicit materialization ordering and exact-set idempotency; and prove incomplete-evidence recovery remains blocked/retryable without a false revision, receipt, or materialization claim.
- **Tests:** add registered-tool E2E fixtures for Tutor → Conceptual Reviewer → user `MODIFY_SYNTHESIS`/renewed review → exact `ACCEPT_DECISION`, no-write snapshots, explicit selection, Planner → non-writing Candidate Executor → deterministic validation → Document Reviewer → guarded publication ordering, duplicate/restart idempotency, and incomplete derived-state/receipt/publication evidence recovery. Record exact manual commands and limitations because no automated runner is configured.
- **Acceptance:** (1) enabled public deliberation persists only scientific state and leaves proposal bytes, manifests, receipts, and managed revisions unchanged; (2) `MODIFY_SYNTHESIS` preserves prior history and requires a fresh exact reviewed synthesis before user-only acceptance; (3) materialization occurs only after explicit frozen selection and in the required Planner → Candidate Executor → validation → Document Reviewer → guarded publication order; (4) exact retries return the same materialization outcome without duplicate records or revisions; (5) incomplete evidence returns `BLOCKED` or `RECOVERY_REQUIRED`, remains retryable/reconcilable, and never fabricates a revision, receipt, or committed materialization; and (6) no task is marked complete without recorded evidence.

### T11.3 — Public conformance regression
- [x] T11.3 — Public conformance regression
- **Files/discovery:** all registered public V2 tool fixtures, existing V2 lifecycle/direct/`DELIBERATE` suites, `tests/paper-proposal-v2-scientific-*.test.mjs`, `.pi/extensions/paper-proposal-v2/exports.ts`, public projection/metadata seams, and the traceability matrix across `proposal.md`, `specs/scientific-reasoning-workflow/spec.md`, `design.md`, and this `tasks.md`.
- **Depends on:** T11.1 and T11.2; all prior T1.0–T10.2 evidence must be revalidated rather than trusted from the false prior completion claim.
- **Implement:** run the final public conformance pass across full V2 boundaries, privacy/public projection, lifecycle/direct/`DELIBERATE` route isolation, default-off compatibility, materialization and recovery boundaries, and proposal/design/task traceability. Correct only conformance gaps exposed by this pass; do not broaden the approved corrective scope.
- **Tests:** execute every available registered-tool, focused scientific, and existing V2 fixture covering lifecycle withdrawal/restore, direct-document operations, `DELIBERATE`, manifests, receipts, audits, locks, recovery, privacy rejection, public projection, route isolation, and exact disabled behavior. Compare results with the proposal, `specs/scientific-reasoning-workflow/spec.md`, `design.md`, and all task acceptance criteria; record exact manual evidence where execution is unavailable.
- **Acceptance:** (1) all full-V2 boundary and compatibility checks pass without scientific side effects on non-scientific routes; (2) public and persisted outputs remain privacy-safe and minimally projected; (3) route isolation and exact default-off behavior are proven at the registered public tool; (4) materialization, idempotency, and incomplete-evidence recovery invariants are proven; (5) the final audit traces every requirement to an implemented, tested, and evidenced task; (6) the false prior `26/26` completion claim is superseded by these unchecked corrective tasks until evidence supports completion; and (7) the user constraint is preserved verbatim: “Amend only `openspec/changes/scientific-reasoning-workflow/tasks.md`; do not modify code, tests, or apply-progress.”
