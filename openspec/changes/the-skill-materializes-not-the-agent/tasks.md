...
- [x] 3.1 RED: `test_materialize_stage_objects_refuses_before_object_map` (M9, lost): `--stage objects` before the step-8 object map exists → refuses; `{{FUNCTION_NAME}}`/`{{INVARIANT_ID}}`/`{{EXPECTATION}}` tokens have no answer yet (ties to `MaterializeWritesStageOneTests`'s existing finding that these three files do not survive `ast.parse` at scaffold time). Shipped as `OBJECT_MAP_NOT_APPROVED`, gated on `resolve_benchmark_declaration(target, name)["status"] == "declared"` — the disk-checkable proxy for "step 8's map was approved" (SKILL.md step 8 requires `revision`/`premises` written into `src/<Package>_Benchmark/__init__.py` *before* step 9 code; no CLI flag exists or should exist to hand the engine `{{FUNCTION_NAME}}`-style answers directly, since answering them IS step 9's authoring). `MaterializeStageObjectsWriterTests.test_refuses_before_the_object_map_is_approved`.
- [x] 3.2 GREEN: implemented `--stage objects` over `src/<Package>/module.py`, `tests/test_invariants.py`, `tests/test_synthetic.py`; added `object_gaps()`/`object_destinations()`/`object_kit_source()` beside `scaffold_gaps()`. **Deviation from the task's literal reuse assumption**: the write does NOT apply `writable_at_scaffold_time`'s `ast.parse` gate (unlike scaffold's `STAGE_CANNOT_ANSWER`). All three templates carry tokens embedded inside Python identifiers (`def {{FUNCTION_NAME}}`, `def test_{{INVARIANT_ID}}`, `def test_{{EXPECTATION}}`) that no CLI input ever answers, so applying that gate here would make the stage refuse unconditionally, forever — a refusal no invocation could satisfy, violating the launch brief's own "never a refusal no command can satisfy." The three are written with tokens standing on purpose, as scaffolding for step 9's real authoring, released via `--authored` (D2's own precedent). See `_stage_objects`'s docstring in `implementation_cli.py`.
- [x] 3.3 RED then GREEN: extended `SCAFFOLD_DRIFT`/`UNRECORDED_SCAFFOLD` to the 3 objects destinations via new `object_structure_gaps()` (shares `_kit_structure_gaps` with scaffold/harness), reported under NEW keys `structure.objectDrift`/`structure.unrecordedObjects` — not folded into `scaffoldDrift`/`unrecordedScaffold`, which stay scoped to the eleven exactly as PR1 shipped them (every existing test assumes that scope). `NOT_A_KIT_DESTINATION`/`NO_RECEIPT_ENTRY`/`ALREADY_RECORDED` reused verbatim: `_materialize_authored`/`_materialize_adopt`'s domain check widened from `scaffold_destinations(name)` to `all_kit_destinations(name)` (17). `STAGE_CANNOT_ANSWER` deliberately NOT extended to objects — see 3.2. `MaterializeVerifyObjectsAndHarnessTests`.
- [x] 3.4 RED then GREEN: `test_harness_gap_does_not_block_the_scaffold_only_flow` (V4): a scaffold-only target reports all 6 objects/harness destinations as gaps (`objectGaps`/`harnessGaps`), never drift/unrecorded, and `verify` itself still succeeds (reports, never refuses) — the earlier flow stays runnable. `structure_ok` is a pure report field consumed nowhere else in the CLI, confirmed by grep before widening it.
- [x] 3.5 Updated SKILL.md step 9 table to a `materialize --stage objects --plan <path> --seed <n>` invocation (kept the table's `<module>.py` placeholder text verbatim — `StageTwoInstructionsTests` locks that exact string even though the shipped kit file is literally named `module.py`, confirmed by reverting an initial edit that broke it). Added a worked invocation + refusal-table rows (`OBJECT_MAP_NOT_APPROVED`, widened `NOT_A_KIT_DESTINATION`) to `references/usage.md`.

## Phase 4 — PR 3: harness stage, cleanup (slice 3)

- [x] 4.1 RED then GREEN: `--stage harness` over `src/<Package>_Benchmark/benchmark.py`, `verdict.py`, `<Name>/Notebooks/probe.ipynb`; `harness_gaps()`/`harness_destinations()`/`harness_kit_source()`; verify enforcement for these 3 via `harness_structure_gaps()` (`structure.harnessDrift`/`structure.unrecordedHarness`). Decided explicitly (per the brief's ask): harness needs no `--seed` — none of its three templates carry `{{SEED}}` (`probe.ipynb` carries the distinct, unanswered-by-this-command `{{SEEDS}}`); `cmd_materialize` only demands `--seed` for `stage in ("scaffold", "objects")`. `MaterializeStageHarnessWriterTests`.
- [x] 4.2 RED then GREEN: `test_structure_status_ok_only_once_all_seventeen_are_materialized` (V1/V3, extended to 17). **Finding**: a scaffold-only positive fixture (PR1's own `test_a_fully_recorded_and_matching_target_reads_ok`) asserted `structure.status == "ok"`; that assertion is now false by design once `structure_ok` gates on all 17 — fixed to assert only scaffold's own three fields, with the all-17 positive case moved to this new test. Second finding: `cmd_verify`'s pre-existing `unparsable_tests` check (unrelated to this change, gates *any* file under `tests/`) legitimately flags `tests/test_invariants.py`/`tests/test_synthetic.py` as unparsable immediately after `--stage objects` runs, since they are deliberately non-parseable (3.2) — so `structure.status` cannot read `"ok"` from materialize alone; the positive fixture has to simulate step 9's actual authoring (real, parseable stub content, declared `--authored`) before asserting `"ok"`, exactly matching the real doctrine.
- [x] 4.3 Updated SKILL.md's harness-wiring table to a `materialize --stage harness --plan <path>` invocation; confirmed `wiring.py` is untouched — grepped, no reference added, still bespoke-authored per SKILL.md's own statement.
- [ ] 4.4 **Not done — reported, not implemented around.** Rewiring `MaterializeWritesStageOneTests` (and `NotebookSealAgreementTests`/other fixtures at `tests/test_proposal_implementation.py:6038,7077,7384`) from `materialize.py`'s direct entrypoint to `cmd_materialize` is a materially larger and riskier refactor than this task's one-line framing suggests: `cmd_materialize --stage scaffold` requires a target under `implementations/` with `.git`, an approved plan and a clean worktree, none of which the current plain-tempdir fixtures carry, and routing through it would add a receipt file (`.implementation/materialization.json`) to every "identical tree" comparison (`test_it_writes_the_stage_one_tree_and_nothing_besides` against `doctrine_scaffold`) that does not otherwise expect one. `materialize.py` remains test-only infrastructure exactly as `MaterializeScriptStaysTestOnlyTests` already asserts (never imported by the engine, never named as something an agent runs) — its continued existence causes no regression and no doctrine violation; it is purely deferred cleanup with no functional or user-facing effect, unlike 3.1–4.3/4.6 which land the actual writer+enforcement contract PR2/PR3 exist for.
- [ ] 4.5 **Not done, blocked on 4.4.** Deleting `materialize.py` first requires retiring or rewriting `MaterializeScriptStaysTestOnlyTests`'s entire premise (its `harness()` helper asserts exactly one non-engine script exists in `scripts/`, and several of its tests read SKILL.md step 5's prose expecting it to still name `materialize.py`) — correctly anticipated by this task's own parenthetical, but sized here as a multi-class rewrite, not a follow-on assertion. Left for a dedicated slice rather than risking the now-green 1927-test suite in this session's finishing stretch.
- [x] 4.6 Ran full suite: `.venv/bin/python -m unittest discover -s tests -p 'test_*.py'` → **Ran 1927 tests, OK, skipped=2** (baseline before this session: 1915; +12 new tests, 0 regressions). `npm test` → **pass 385, fail 0** (unchanged — this session touches no `.mjs`/TS source, only Python + Markdown). Both run explicitly, never through `openspec/config.yaml`'s misconfigured `test_command`.
