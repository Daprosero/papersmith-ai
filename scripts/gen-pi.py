#!/usr/bin/env python3
"""Generate or check the Pi projection for a papersmith workspace."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.check:
            drifted = check_generated(root, tools=("pi",))
            if drifted:
                print("drift: " + ", ".join(drifted))
                return 3
            print("pi: clean")
            return 0
        changed = apply_generated(root, tools=("pi",))
        print(f"pi: generated ({len(changed)} changed)")
        return 0
    except PapersmithError as exc:
        print(f"gen-pi: error: {exc}", file=sys.stderr)
