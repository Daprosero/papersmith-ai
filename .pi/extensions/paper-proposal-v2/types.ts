...
export const PARSER_VERSION = 'paper-proposal-v2/1';
...
export type StructuralEntry = { entryId:string; type:EntryType; startByte:number; endByte:number; textSha256:string; parentId?:string; childIds:string[]; ordinal:number; headingPath:string[]; labels:string[]; tags:string[]; lexicalTerms:string[]; deterministicAliases:string[]; neighboringEntryIds:string[] };
...
export type CompositeTarget = { entryIds:string[]; startByte:number; endByte:number; documentSha256:string; textSha256:string; exactProvidedText:string };
...
export type LocalContext = { documentSha256:string; targetEntryId:string; instruction:string; fragments:ContextFragment[]; nearbySymbols:Record<string,string>; directReferences:string[]; fidelity?:FidelityConstraints; maxBytes?:number };
...
export type EditPlan = { planVersion:'2'; documentSha256:string; intent:Intent; instructionHash:string; resolvedTargets:string[]; destination?:string; semanticChange:boolean; destructiveIntent:boolean; cleanupLevel:CleanupLevel; constraints:string[]; actions:EditAction[]; expectedEffects:string[]; unresolvedQuestions:string[] };
...
export type RevisionReceipt = { sourceRevision:string; targetRevision:string; sourceFilename:string; targetFilename:string; intent:Intent; operation:Intent; instructionHash:string; documentShaBefore:string; documentShaAfter:string; resolvedEntryIds:string[]; patchIds:string[]; patchCount:number; cleanupLevel:CleanupLevel; derivedStateStatus:DerivedStateStatus; modelCalls:number; roleAuthorizations:number; inputTokens:number; outputTokens:number; elapsedMs:number; parallelTasks:number; validationResults:Record<string,boolean> };
...
export type { ScientificThread, ScientificDecision, ScientificEvent, ThreadRelation, ThreadSynthesis, ScientificAct, ScientificActResolution, BoundedScientificSeed, ThreadTransitionIntent, ThreadResolution, ProjectEntryState, ScientificWorkflowOperation, ScientificWorkflowPublicStatus, ScientificActKind, ScientificThreadStatus, ScientificDecisionStatus, ScientificEventType, ThreadRelationKind, ThreadSynthesisStatus, MaterializationState, MaterializationPlanKind, FrozenDecisionSelection, MaterializationRecord, MaterializationReservedDecision, MaterializationClaimProvenance, MaterializationPlan, ScientificActorKind, ConceptualReviewOutcome, ScientificResolutionStatus, ScientificAuditStatus, ScientificWorkflowRequest, ScientificWorkflowPublicResult } from './scientific-domain.js';
