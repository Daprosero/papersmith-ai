...
function immutableRelativePath(publicRelativePath:string) { return publicRelativePath.startsWith('proposals/')?`artifacts/${publicRelativePath}`:`artifacts/${publicRelativePath.replace(/^\.paper-proposal-v2\//,'')}`; }
...
export function lifecyclePublicRelativePaths(filename:string) {
 ...
 return [`proposals/${filename}`,`.paper-proposal-v2/state/${filename}.json`,`.paper-proposal-v2/receipts/${filename}.json`] as const;
}

export function withdrawalRootPath(root:string) { return join(resolve(root),'.paper-proposal-v2','withdrawn'); }
