...
`verify` returns, separately. There are nineteen of them, and a list written
...
| `unreachableLadder` | A declared ladder the sequence beside it can never climb to the launch floor of: `levels`, `requiredLevel`, `highestAttainable`, `cappedBy`, and the consequence. Fires from four rungs up with a leveled `@rehearsal` item, which `smokeReady` ceilings at the floor plus one | Never — reported so a `RUNG_NOT_ATTAINED` nothing can answer is legible before it is met |
