 ...
 * well-formed `kind:'replace'` decision (design `sdd/paper-proposal-ambient-model`,
...
/** Remaps one relocation decision's own declared entryId field(s) from the ORIGINAL (pre-in-place-publish) resolved entryId to the FRESHLY re-resolved entryId at the SAME locus after the in-place half publishes (design `sdd/paper-proposal-ambient-model`, SLICE 1b) -- byte-offset-derived composite entryIds are never stable across a publish, only the fuzzy locus query is. */
...
export class PaperProposalOrchestrator {
  ...
  * (design `sdd/paper-proposal-ambient-model`, SLICE 1b): bypasses
