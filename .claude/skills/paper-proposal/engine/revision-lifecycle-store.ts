...
function immutableRelativePath(publicRelativePath:string) { return publicRelativePath.startsWith('proposals/')?`artifacts/${publicRelativePath}`:`artifacts/${publicRelativePath.replace(/^\.paper-proposal\//,'')}`; }
...
export function lifecyclePublicRelativePaths(filename:string) {
 ...
 return [`proposals/${filename}`,`.paper-proposal/state/${filename}.json`,`.paper-proposal/receipts/${filename}.json`] as const;
}

export function withdrawalRootPath(root:string) { return join(resolve(root),'.paper-proposal','withdrawn'); }
