...
export type RevisionLifecycleTransactionDependencies={
 ...
 selfAudit?:typeof runPaperProposalSelfAudit;
...
function markerRelative(operationId:string) { return `.paper-proposal/withdrawn/${operationId}/audit-marker.json`; }
...
async function prepareWithdrawalRoot(fs:LifecycleFs,root:string) {
 await requireExactDirectory(fs,join(root,'.paper-proposal'));
...
export class RevisionLifecycleTransaction {
 ...
 private readonly selfAudit:typeof runPaperProposalSelfAudit;
 constructor(private readonly projectRoot:string,private readonly dependencies:RevisionLifecycleTransactionDependencies={}) {
  ...
  this.selfAudit=dependencies.selfAudit??runPaperProposalSelfAudit;
 }
