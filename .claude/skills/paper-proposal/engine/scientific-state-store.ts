...
function statePath(root: string) { return join(root, '.paper-proposal', 'scientific'); }
...
export class ScientificStateStore {
	...
	private async ensureLayout(root: string) {
		const paths = layout(root);
		for (const directory of [join(root, '.paper-proposal'), paths.scientific, paths.events, paths.materializations, paths.transactions, paths.projections]) {
