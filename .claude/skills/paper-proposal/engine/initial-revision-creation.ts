...
function initialRevisionLockPath(root: string): string {
	return join(root, '.paper-proposal', 'locks', 'initial-revision.lock');
}
 ...
 * `.paper-proposal/state/<filename>.json` COMMITTED manifest and `.paper-proposal/receipts/<filename>.json`
