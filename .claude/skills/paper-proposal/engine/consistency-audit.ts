...
function expectedMarkerPath(operationId:string) { return `.paper-proposal/withdrawn/${operationId}/audit-marker.json`; }
...
function validateArtifactList(artifacts:unknown,operationType:unknown,failures:string[],prefix:string):artifacts is PendingAuditArtifact[] {
  ...
  for (const artifact of artifacts) {
    ...
    const match=artifact.publicRelativePath.match(/^(?:proposals\/(.+)|\.paper-proposal\/(?:state|receipts)\/(.+)\.json)$/);
    const candidate=match?.[1]??match?.[2];
    if (!candidate||!MANAGED.test(candidate)||filename&&candidate!==filename) valid=false;
    else filename=candidate;
  }
  if (filename) {
    const expected=new Set([`proposals/${filename}`,`.paper-proposal/state/${filename}.json`,`.paper-proposal/receipts/${filename}.json`]);
...
function immutableRelativePath(publicRelativePath:string) {
  ...
  return `artifacts/${publicRelativePath.replace(/^\.paper-proposal\//,'')}`;
}
...
async function auditWithdrawals(root:string,context:PendingAuditContext|undefined,failures:string[]) {
  ...
  let entries;
  try {
    withdrawn=await resolveDirectoryWithin(root,join(root,'.paper-proposal','withdrawn'));
    ...
    if (context) failures.push(`PENDING_AUDIT_OPERATION_NOT_FOUND:${context.operationId}`);
      ...
      try { await lstat(join(root,'.paper-proposal','withdrawn')); failures.push('WITHDRAWAL_ROOT_UNSAFE'); }
      catch {}
    }
    return;
  }
...
export async function runConsistencyAudit(input:{projectRoot:string;auditContext?:PendingAuditContext}) {
  ...
  for (const directory of ['state','receipts']) {
    const path=join(root,'.paper-proposal',directory);
    try {
