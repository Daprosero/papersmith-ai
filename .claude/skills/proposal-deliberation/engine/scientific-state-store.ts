...
function statePath(root: string) { return join(root, '.proposal-deliberation', 'scientific'); }
...
export class ScientificStateStore {
	...
	private async ensureLayout(root: string) {
		const paths = layout(root);
		for (const directory of [join(root, '.proposal-deliberation'), paths.scientific, paths.events, paths.materializations, paths.transactions, paths.projections]) {
