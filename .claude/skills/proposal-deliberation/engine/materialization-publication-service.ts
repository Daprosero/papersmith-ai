...
import type { MaterializationOutcome, MaterializationRecord } from './scientific-domain.js';
...
export class MaterializationPublicationService {
	...
	private async verifyAndCommit(record: MaterializationRecord, plan: NonNullable<MaterializationRecord['plan']>, candidate: ExactDocumentCandidate, published: Awaited<ReturnType<ProposalWorkspaceAdapter['publishInitial']>> | Awaited<ReturnType<ProposalWorkspaceAdapter['publishApprovedSuccessor']>>): Promise<MaterializationPublicationResult> {
		...
		if (!candidateMatches || sha256(published.publishedBytes) !== published.publishedSha256) return this.recoveryRequired(record, 'MATERIALIZATION_PUBLISHED_BYTES_MISMATCH', ['published_bytes:inconsistent']);
		try {
			...
			if (derived.documentSha256 !== published.publishedSha256) return this.recoveryRequired(record, 'MATERIALIZATION_DERIVED_STATE_MISMATCH', ['derived_state:inconsistent']);
			...
			if (!verifiedDerived) return this.recoveryRequired(record, 'MATERIALIZATION_PUBLICATION_EVIDENCE_INCOMPLETE', ['derived_state:absent']);
			let receiptBytes: Buffer;
			try {
				receiptBytes = await readFile(receiptPath(this.dependencies.projectRoot, published.targetFilename));
			} catch {
				return this.recoveryRequired(record, 'MATERIALIZATION_PUBLICATION_EVIDENCE_INCOMPLETE', ['receipt:absent']);
			}
			if (sha256(receiptBytes) !== sha256(Buffer.from(JSON.stringify(receipt)))) return this.recoveryRequired(record, 'MATERIALIZATION_PUBLICATION_EVIDENCE_INCOMPLETE', ['receipt:inconsistent']);
			...
			try { await (this.dependencies.derivedStore ?? { markDerivedState }).markDerivedState(this.dependencies.projectRoot, published.targetFilename, 'FAILED', 'MATERIALIZATION_PUBLICATION_EVIDENCE_INCOMPLETE'); } catch {}
			return this.recoveryRequired(record, 'MATERIALIZATION_PUBLICATION_EVIDENCE_INCOMPLETE', ['confirmation:unavailable']);
		}
	}

	private async recoveryRequired(record: MaterializationRecord, code: string, evidence: string[]): Promise<MaterializationPublicationResult> {
		return this.outcome(record, 'RECOVERY_REQUIRED', {
			code,
	...
	private async outcome(record: MaterializationRecord, state: 'BLOCKED' | 'RECOVERY_REQUIRED', outcome: string | MaterializationOutcome): Promise<MaterializationPublicationResult> {
		const persisted = await this.dependencies.store.recordMaterializationOutcome(record, state, outcome);
		const code = typeof outcome === 'string' ? outcome : outcome.code;
		if (persisted.status !== 'ready') {
			return state === 'RECOVERY_REQUIRED'
				? { status: 'recovery_required', code: 'MATERIALIZATION_RECOVERY_TRANSITION_FAILED' }
				: { status: 'blocked', code: 'MATERIALIZATION_BLOCKED_TRANSITION_FAILED' };
		}
		return state === 'RECOVERY_REQUIRED' ? { status: 'recovery_required', code } : { status: 'blocked', code };
	}
}
