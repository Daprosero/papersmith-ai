...
const extension=await readFile(path.resolve('.claude/skills/paper-proposal/engine/proposal-workspace.ts'),'utf8');
test('registers exactly one V2 proposal editing orchestrator',()=>{assert.match(extension,/PaperProposalOrchestrator/);assert.equal((extension.match(/new PaperProposalOrchestrator/g)??[]).length,1);});
