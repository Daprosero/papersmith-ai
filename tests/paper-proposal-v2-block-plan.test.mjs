...
test('preflight diagnostic ordering is locale-independent', async () => {
	const script = `
		...
		const piRoot = process.env.PAPER_PROPOSAL_PI_ROOT;
		const { createJiti } = await import(pathToFileURL(path.join(piRoot, 'node_modules/jiti/lib/jiti.mjs')).href);
		const jiti = createJiti(import.meta.url, { alias: {
			'@earendil-works/pi-coding-agent': path.join(piRoot, 'dist/index.js'),
			'@earendil-works/pi-ai/compat': path.join(piRoot, 'node_modules/@earendil-works/pi-ai/dist/compat.js'),
		...
		const v2 = await jiti.import(process.env.PAPER_PROPOSAL_EXPORTS);
	...
	const diagnosticsForLocale = async (locale) => {
		const { stdout } = await execFileAsync(process.execPath, ['--input-type=module', '--eval', script], {
			env: { ...process.env, LANG: locale, LC_ALL: locale, PAPER_PROPOSAL_PI_ROOT: piRoot, PAPER_PROPOSAL_EXPORTS: exportsPath },
		});
