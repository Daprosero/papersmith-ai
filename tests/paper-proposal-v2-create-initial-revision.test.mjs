...
// Chain under test: cli.mjs's registered `paper_proposal_execute` tool dispatches
// operation:'CREATE_INITIAL_REVISION' -> proposal-workspace.ts's dedicated route
// (independent of PAPER_PROPOSAL_SCIENTIFIC_WORKFLOW_ENABLED and CANONICAL_METADATA_UNAVAILABLE)
...
// ---------------------------------------------------------------------------
// Reachability: the real `paper_proposal_execute` tool dispatch (cli.mjs's exact call
// contract), independent of PAPER_PROPOSAL_SCIENTIFIC_WORKFLOW_ENABLED. Uses a mkdtemp()
// temp project root only -- never the real repository `proposals/` directory.
// ---------------------------------------------------------------------------

async function fixture() {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'paper-proposal-create-initial-'));
...
test('CREATE_INITIAL_REVISION is reachable via the registered tool and produces a managed r01 when no managed proposal exists, independent of the scientific-workflow flag', async () => {
	const before = process.env.PAPER_PROPOSAL_SCIENTIFIC_WORKFLOW_ENABLED;
	...
	try {
		...
		await run.dispose();
		if (before !== undefined) process.env.PAPER_PROPOSAL_SCIENTIFIC_WORKFLOW_ENABLED = before;
	}
});
...
test('CREATE_INITIAL_REVISION writes derived-state and receipt sidecars in the same layout as ordinary materialization, so a fresh r01 yields a CONSISTENT inventory', async () => {
	...
	try {
		...
		const stateBytes = await readFile(path.join(run.projectRoot, '.paper-proposal', 'state', `${result.targetFilename}.json`), 'utf8');
		...
		const receiptBytes = await readFile(path.join(run.projectRoot, '.paper-proposal', 'receipts', `${result.targetFilename}.json`), 'utf8');
