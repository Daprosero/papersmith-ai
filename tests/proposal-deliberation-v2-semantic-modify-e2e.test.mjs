...
const {createJiti}=await import('jiti'); const jiti=createJiti(import.meta.url); const workspaceModule=await jiti.import(path.resolve('skills/_core/deliberation/engine/proposal-workspace.ts')); const v2=await jiti.import(path.resolve('skills/_core/deliberation/engine/exports.ts'));
const realHashes={r01:'db8ad910633a14b53306883bf152a5ea70e37d1349ba073f0a4dcac152b0fae3'};
...
async function realFixture(){const root=await mkdtemp(path.join(os.tmpdir(),'pp-v2-semantic-real-'));await mkdir(path.join(root,'proposals'),{recursive:true});await copyFile(path.resolve('tests/fixtures/research-concept-r01.md'),path.join(root,'proposals/research-concept-r01.md'));const guard=workspaceModule.createDocumentOperationGuard(root);const workspace=workspaceModule.createProposalWorkspaceTool(root,{operationGuard:guard});return {root,adapter:new v2.ProposalWorkspaceAdapter(root,guard,workspace,()=>`semantic-real-${Math.random()}`)}}
...
$$`;
 ...
 const result=await new v2.ProposalDeliberationOrchestrator(root,adapter,undefined,planner).execute({sourceFilename:'research-concept-r01.md',instruction});
...
test('non-contiguous equivalent equations are ambiguous before planner',async()=>{
 ...
 const result=await new v2.ProposalDeliberationOrchestrator(root,adapter,undefined,{async plan(){calls++;return {}}}).execute({instruction:`Reemplaza exactamente este bloque:\n${target}\npor este bloque:\n$$\nC=3.\n$$`});
