...
async function project() { return mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-store-')); }
...
test('ScientificStateStore persists only versioned authoritative records and rebuilds its projection', async () => {
	...
	const scientific = path.join(rootDir, '.proposal-deliberation/scientific');
...
test('ScientificStateStore rejects unsafe symlinks and does not follow them as records', async () => {
	...
	const store = new v2.ScientificStateStore(rootDir);
	await seed(store);
	const manifest = path.join(rootDir, '.proposal-deliberation/scientific/manifest.json');
...
test('ScientificStateStore keeps an interrupted transition fail-closed and recovery never invents state', async () => {
	...
	assert.equal(await nativeFs.stat(path.join(rootDir, '.proposal-deliberation/scientific/transactions/transition-1.json')).then(() => true), true);
});
...
test('ScientificStateStore validates replay and can clean only a fully committed marker', async () => {
	...
	const store = new v2.ScientificStateStore(rootDir);
	const committed = await seed(store);
	const markerPath = path.join(rootDir, '.proposal-deliberation/scientific/transactions/replay.json');
	...
	const eventsPath = path.join(rootDir, '.proposal-deliberation/scientific/events/1-event-1.json');
...
test('scientific persistence imports canonical contracts and keeps storage-only records local', async () => {
	const source = await readFile(path.join(root, '.claude/skills/proposal-deliberation/engine/scientific-state-store.ts'), 'utf8');
