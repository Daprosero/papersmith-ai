// paper-proposal-tutor-repair (deliberated-operations, SLICE 2): deliberated
...
// design `sdd/paper-proposal-tutor-repair/design-deliberated-operations` and
...
async function serviceFixture({ tutorResults, reviewerResults = [reviewerAssessment()], documentFragments = [], instruction = 'Synthesize this bounded scientific question.' } = {}) {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'paper-proposal-deliberated-mc-'));
...
async function withTempRoot(run) {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'paper-proposal-deliberated-mc-live-'));
	try {
