// proposal-deliberation-tutor-repair (V2-scientific structured edits): closes the
...
async function serviceFixture({ tutorResults, reviewerResults = [reviewerAssessment()], documentFragments = [] } = {}) {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-structured-'));
...
test('an old-style TUTOR_ASSESSED event with no proposedEdit key still validates (additive-optional backward compat)', async () => {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-structured-legacy-'));
	try {
		const store = new v2.ScientificStateStore(projectRoot);
...
test('proposedEdit.replacementText accepts up to ~20,000 bytes (raised cap) while every other public string stays bounded at 2,000', async () => {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-structured-cap-'));
	try {
...
test('proposedEdit.replacementText is still bounded (not unlimited) at ~20,000 bytes', async () => {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-structured-cap-over-'));
	try {
...
test('the raised cap never weakens the 2,000-byte guard for any OTHER public payload string, including other fields nested inside proposedEdit', async () => {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-structured-cap-other-'));
	try {
	...
	const projectRoot2 = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-structured-cap-other2-'));
	try {
...
async function withTempRoot(run) {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-structured-live-'));
	try {
