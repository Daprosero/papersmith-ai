...
async function project() {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-audit-'));
...
test('scientific audit projects absent, validated, and stale revision evidence without mutating lifecycle inventory', async () => {
	...
	assert.equal((await v2.runProposalDeliberationSelfAudit({ projectRoot: rootDir })).checks.find((check) => check.id === 'scientific-consistency').status, 'PASS');
