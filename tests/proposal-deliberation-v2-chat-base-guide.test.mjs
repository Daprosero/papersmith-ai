...
// `createProposalDeliberationExtension`'s `tutor` option and `ChatDeliberationService`'s
...
async function fixture() {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-chat-base-guide-'));
	...
	const tools = [];
	workspace.createProposalDeliberationExtension({ projectRoot, operationGuard: guard, tutor })({ registerTool: (candidate) => tools.push(candidate), on: () => {} });
