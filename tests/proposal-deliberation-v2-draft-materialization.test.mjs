...
test('missing or empty materializationPayload fails closed before resolving or creating any file', async () => {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-empty-draft-'));
...
function registerPublicDraftExtension({ run, sessionIdentity, tutor, draftRegistry, operationGuard }) {
	...
	workspace.createProposalDeliberationExtension({
...
// (`{assess}`) is injected directly through `createProposalDeliberationExtension`'s `tutor`
...
test('public materialization without any produced draft returns CHAT_CONTENT_REQUIRED before guards or filesystem', async () => {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-no-chat-draft-'));
	let inventoryCalls = 0;
	const guardCalls = [];
	const guard = workspace.createDocumentOperationGuard(projectRoot);
	const originalExecute = guard.execute.bind(guard);
...
test('public CHAT_DELIBERATION hands off consolidated bytes exactly once to guarded DOCUMENT_EDIT materialization', async () => {
	...
	try {
		const tools = [];
		workspace.createProposalDeliberationExtension({ projectRoot: run.projectRoot, operationGuard: workspace.createDocumentOperationGuard(run.projectRoot), draftMaterialization: { managedDocumentPath: run.primaryRoute, draftDirectory: 'working-drafts', allowedExtensions: ['.md'] }, tutor })({ registerTool: tool => tools.push(tool), on: () => {} });
		const tool = tools.find(candidate => candidate.name === 'proposal_deliberation_execute');
