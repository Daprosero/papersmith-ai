...
$$`;
 ...
 const result=await new v2.PaperProposalV2Orchestrator(root,adapter,undefined,planner).execute({sourceFilename:'research-concept-r01.md',instruction});
...
test('non-contiguous equivalent equations are ambiguous before planner',async()=>{
 ...
 const result=await new v2.PaperProposalV2Orchestrator(root,adapter,undefined,{async plan(){calls++;return {}}}).execute({instruction:`Reemplaza exactamente este bloque:\n${target}\npor este bloque:\n$$\nC=3.\n$$`});
