...
async function fixture({ incompleteEvidence = false, lifecycleV1WorkspaceId } = {}) {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-public-'));
	...
	const calls = [];
	let id = 0;
	const extension = workspace.createProposalDeliberationExtension({
		projectRoot,
		scientificWorkflow: {
...
async function scientificPaths(projectRoot) {
	try { return (await readdir(path.join(projectRoot, '.proposal-deliberation/scientific'), { recursive: true })).sort(); } catch { return []; }
}
...
test('disabled gate returns exact SCIENTIFIC_WORKFLOW_DISABLED without dependency invocation or persistence', async () => {
	...
	} finally { if (previous === undefined) delete process.env.PROPOSAL_DELIBERATION_SCIENTIFIC_WORKFLOW_ENABLED; else process.env.PROPOSAL_DELIBERATION_SCIENTIFIC_WORKFLOW_ENABLED = previous; await run.dispose(); }
});
...
test('public scientific projection rejects private, infrastructure, path, stack, full-document, and unpublished-candidate input without persistence', async () => {
	...
	try {
		process.env.PROPOSAL_DELIBERATION_SCIENTIFIC_WORKFLOW_ENABLED = 'true';
...
test('initial public deliberation routes through Tutor then Reviewer and persists scientific state only', async () => {
	...
	try {
		...
		assert.deepEqual(await readdir(path.join(run.projectRoot, '.proposal-deliberation')), ['scientific']);
...
test('public REQUEST_MATERIALIZATION exercises the frozen plan, review, guarded publication, and public outcome', async () => {
	...
	try {
		...
		assert.ok(await readFile(path.join(run.projectRoot, '.proposal-deliberation/receipts/research-concept-r01.md.json')));
	} finally { if (previous === undefined) delete process.env.PROPOSAL_DELIBERATION_SCIENTIFIC_WORKFLOW_ENABLED; else process.env.PROPOSAL_DELIBERATION_SCIENTIFIC_WORKFLOW_ENABLED = previous; await run.dispose(); }
});
