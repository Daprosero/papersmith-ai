...
const publicPaths=[`proposals/${r02}`,`.proposal-deliberation/state/${r02}.json`,`.proposal-deliberation/receipts/${r02}.json`];

function immutablePath(publicRelativePath) {
  return publicRelativePath.startsWith('proposals/')?`artifacts/${publicRelativePath}`:`artifacts/${publicRelativePath.replace(/^\.proposal-deliberation\//,'')}`;
}
...
async function fixture() {
  ...
  const operationDirectory=path.join(root,'.proposal-deliberation','withdrawn',operationId);
  const artifacts=[];
  ...
  const context={
    ...
    expectedMarker:{relativePath:`.proposal-deliberation/withdrawn/${operationId}/audit-marker.json`,state:'PENDING_AUDIT',inventoryDigest,expiresAt},
  };
  const marker={
...
async function runActiveSelfAudit(value,run=()=>v2.runProposalDeliberationSelfAudit({projectRoot:value.root,auditContext:value.context})) {
...
test('lifecycle-v1 legacy diagnostics leave an active PENDING_AUDIT fixture byte-identical and do not create authority',async()=>{
  ...
  await assert.rejects(readdir(path.join(value.root,'.proposal-deliberation','lifecycle','v1')),{code:'ENOENT'});
...
test('a finalized committed marker passes fresh context-free audits',async()=>{
  ...
  const selfAudit=await v2.runProposalDeliberationSelfAudit({projectRoot:value.root});
