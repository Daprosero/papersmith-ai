...
async function fixture(dependencies={}) {
 ...
 await mkdir(path.join(root,'.proposal-deliberation'),{recursive:true});
 let id=0;
...
test('registers one immutable base with stable identity and durable content',async()=>{
 ...
 assert.match(base.storageRoot,new RegExp(`${path.sep.replace(/\\/g,'\\\\')}\.proposal-deliberation${path.sep.replace(/\\/g,'\\\\')}lifecycle${path.sep.replace(/\\/g,'\\\\')}v1`));
});
...
test('public lifecycle routing uses configured v1 authority without legacy proposal files',async()=>{
 ...
 assert.doesNotMatch(JSON.stringify(withdrawn.transitionEvidence),/Title|Unchanged paragraph|withdraw research-concept|proposal-deliberation-lifecycle-v1-|absolute|contents\//i);
 ...
 const transitions=await Promise.all((await readdir(path.join(root,'.proposal-deliberation','lifecycle','v1','transitions'))).map((name)=>readFile(path.join(root,'.proposal-deliberation','lifecycle','v1','transitions',name),'utf8')));
 assert.doesNotMatch(transitions.join('\n'),/Title|Unchanged paragraph|withdraw research-concept|proposal-deliberation-lifecycle-v1-/);
...
test('ignores pre-marker staging, reconstructs committed marker state, and fails closed for contradictory durable records',async()=>{
 ...
 const first=await service.createFromBase({workspaceId:'workspace-1',requestId:'create-1',operation:'CREATE_FROM_BASE',revisionId:'revision-1',source:{sourceKind:'BASE_DOCUMENT',sourceId:'base-1',sourceContentHash:base.base.contentHash,baseDocumentId:'base-1'},approvedChanges:[]});
 await service.createSuccessor({workspaceId:'workspace-1',requestId:'create-2',operation:'CREATE_SUCCESSOR',revisionId:'revision-2',source:{sourceKind:'REVISION',sourceId:'revision-1',sourceContentHash:first.revision.contentHash,baseDocumentId:'base-1'},approvedChanges:[]});
 const transitions=path.join(root,'.proposal-deliberation','lifecycle','v1','transitions');
...
test('fails closed for an orphan withdrawal record instead of treating it as history',async()=>{
 ...
 await writeFile(path.join(root,'.proposal-deliberation','lifecycle','v1','withdrawals','withdrawal-orphan.json'),JSON.stringify(orphan));
 await assert.rejects(new v2.LifecycleService(root).rebuildLifecycleInventory('workspace-1'),/LIFECYCLE_INVENTORY_INCONSISTENT/);
});
...
test('persists request and result records before exposing a committed lifecycle result',async()=>{
 ...
 const request=JSON.parse(await readFile(path.join(root,'.proposal-deliberation','lifecycle','v1','requests','register-1.json'),'utf8'));
 const storedResult=JSON.parse(await readFile(path.join(root,'.proposal-deliberation','lifecycle','v1','results','register-1.json'),'utf8'));
...
test('fails closed for missing content, broken lineage, and cross-workspace durable evidence',async()=>{
 ...
 await rm(path.join(missing.root,'.proposal-deliberation','lifecycle','v1','contents',missing.base.base.contentHash));
 ...
 const revisionPath=path.join(broken.root,'.proposal-deliberation','lifecycle','v1','revisions','revision-1.json');
 ...
 const crossPath=path.join(cross.root,'.proposal-deliberation','lifecycle','v1','revisions','revision-1.json');
...
test('exposes explicitly registered lifecycle-v1 inventory read-only without consulting legacy filenames',async()=>{
 ...
 const before=await (await import('node:fs/promises')).readdir(path.join(root,'.proposal-deliberation','lifecycle','v1'),{recursive:true});
 ...
 assert.deepEqual(await (await import('node:fs/promises')).readdir(path.join(root,'.proposal-deliberation','lifecycle','v1'),{recursive:true}),before);
});
...
test('legacy workspaces are diagnostic-only and lifecycle-v1 reads never create authority records',async()=>{
 ...
 const withdrawn=path.join(root,'.proposal-deliberation','withdrawn');
