...
const workspaceModule = await jiti.import(path.join(root, '.claude/skills/proposal-deliberation/engine/proposal-workspace.ts'));
...
async function fixture({ successor = false, reviewerDecision = 'APPROVE', adapter, derivedStore } = {}) {
	const projectRoot = await mkdtemp(path.join(tmpdir(), 'proposal-deliberation-scientific-publication-'));
...
test('approved r01 candidate publishes only through INITIAL_CREATE then commits verified derived state, minimal receipt, and provenance', async () => {
	...
	const receipt = JSON.parse(await readFile(path.join(projectRoot, '.proposal-deliberation/receipts/research-concept-r01.md.json'), 'utf8'));
...
test('approved successor uses the guarded V2 successor adapter and commits only the reviewed exact bytes', async () => {
	...
	const receipt = JSON.parse(await readFile(path.join(projectRoot, '.proposal-deliberation/receipts/research-concept-r02.md.json'), 'utf8'));
	assert.equal(receipt.documentShaAfter, createHash('sha256').update(bytes).digest('hex'));
