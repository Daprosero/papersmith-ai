...
const v2 = await jiti.import(path.join(repositoryRoot, '.claude/skills/paper-proposal/engine/exports.ts'));
...
async function tempProjectRoot() {
	return mkdtemp(path.join(os.tmpdir(), 'paper-proposal-revision-resolver-'));
}
...
test('PaperProposalOrchestrator.execute() surfaces MULTIPLE_ACTIVE_REVISIONS instead of NO_MANAGED_PROPOSAL when the managed proposal base is ambiguous', async () => {
	const root = await tempProjectRoot();
	await writeCommittedRevision(root, 'research-concept-r01.md', '# Concept A\n\nFirst idea.\n');
	await writeCommittedRevision(root, 'research-concept-idea-b-r01.md', '# Concept B\n\nSecond idea.\n');
	const adapter = new v2.ProposalWorkspaceAdapter(root, { execute: async () => ({ decision: 'allowed' }) }, { execute: async () => ({}) });
	const orchestrator = new v2.PaperProposalOrchestrator(root, adapter);
