// Multi-section CREATE_SUCCESSOR coverage (paper-proposal-tutor-repair, design
// amendment: multi-section successor + growth advisory).
//
// Extends tests/paper-proposal-successor-locus-inference.test.mjs (tasks 1.3/1.4,
...
// `PaperProposalOrchestrator` over a real temp-dir workspace; no proposal .md is
...
test('published successor receipt instructionHash reflects the full multi-target set, not only the last resolved target (re-audit cleanup)', async () => {
	...
	const orchestratorAB = new v2.PaperProposalOrchestrator(rootAB, adapterAB, undefined, planner);
	...
	const orchestratorAG = new v2.PaperProposalOrchestrator(rootAG, adapterAG, undefined, planner);
